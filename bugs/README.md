...
| D-11 | D | 高 | processDetail 未处理返回值，锁失败时不触发失败标记 | 已修复 | [D-011](./D-011-process-detail-no-result-check.md) |
| E-23 | E | 高 | sendDeductionNotify 未检查 consume 是否为 null | 已修复 | [E-023](./E-023-notify-consume-null.md) |
| B-6 | B | 高 | transDeduction 缺少 MAC 幂等校验 | 已修复 | [B-006](./B-006-deduction-no-mac-idempotent.md) |
| BC-6 | B+C | 高 | FrozenTrans/UnFrozenTrans findFirst().get() 未处理空 | 已修复 | [BC-006](./BC-006-findfirst-no-empty-check.md) |
| BC-7 | B+C | 高 | liteflowResponse.getCause() 为 null 时异常处理不当 | 已修复 | [BC-007](./BC-007-liteflow-cause-null.md) |
