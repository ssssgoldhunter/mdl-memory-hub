...
| 平台付款 | `transPlatformPay` | POST | 自有资金池平台付款，交易类型 `MC` |
| 平台收款 | `transPlatformReceive` | POST | 自有资金池平台收款/垫资扣款，交易类型 `MR` |
| 平台扣款 | `transPlatformDeduction` | POST | 当前复用平台收款逻辑 |

### 2.1.1 自有资金池垫资 Consume Feign

清结算系统调用 `fund-catering-consume` 时，以 `TransConsumeApi` 为准：

| 场景 | Feign 方法 | 接口路径 | 请求对象 | 返回对象 | 调用规则 |
|------|------------|----------|----------|----------|----------|
| 清结算垫资 | `transTransferInner` | `/consume/trans/transTransferInner` | `ConsumeTransferRequest` | `DefaultResult<TransferTransRes>` | 内部转账记账，交易类型 `AT`，不要调用普通 `/transTransfer` |
| 垫资扣款 | `transPlatformReceive` | `/consume/trans/transPlatformReceive` | `ConsumeDeductionTransRequest` | `DefaultResult<TransConsumeRes>` | 平台自有资金收款，交易类型 `MR`，不要调用普通 `/transDeduction` |

公共口径：
- 入参金额 `amount` 是“分”的整数字符串。
- 自有资金池垫资固定使用 `04`。
- `transNo` 由清结算侧传入且必须唯一。
- 平台收款必须传 `platformCardCode`、`platformBankEAccountId`、`platformDealType`、`platformFundTp`。
- 完整字段和示例见 `../docs/SELF_FUND_ADVANCE_CONSUME_API.md`。
